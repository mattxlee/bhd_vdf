...
protected:
  ...
  void SetUp() override {
    srv.reset(new net::Server(
        ioc_,
        tcp::endpoint(asio::ip::address::from_string(SZ_LOCAL_HOST), TEST_PORT),
...
TEST_F(ServerTest, PingPong) {
  srv->set_session_message_handler(
  ...
  client.set_message_handler([&client](
                                 net::Message const* msg, uint8_t msg_id) {
  ...
  client.set_error_handler(
      [](boost::system::error_code ec, net::ActionType type) {
